...
# -*- coding: utf-8 -*-
"""
Codex Provider Switcher V1.24 — macOS Edition
...
def _keychain_error_status(exc):
    """从 keyring 的异常链中提取 macOS Security.framework OSStatus。"""
    visited = set()
    current = exc
    while isinstance(current, BaseException) and id(current) not in visited:
        ...
        for arg in current.args:
            match = re.search(r"(?:^|[,(\s])(-25\d{3})(?:$|[,)\s])", str(arg))
            if match:
...
def _read_keychain_secret_strict(name):
    value = keyring.get_password(KEYCHAIN_SERVICE, name)
    return value or None


def _update_keychain_secret_in_place(name, value):
    """原地更新旧钥匙串条目，保留 ACL，兼容 errSecInvalidOwnerEdit。"""
    if sys.platform != "darwin":
        raise RuntimeError("仅 macOS 支持钥匙串原地更新")
    ...
    attributes = api.create_query(kSecValueData=value)
    status = sec_item_update(query, attributes)
    api.Error.raise_for_status(status)


def set_keychain_secret(name, value):
    """安全写入登录钥匙串；旧签名 ACL 冲突时改为原地更新。"""
    name = validate_env_name(name)
    if not value:
        return
    try:
        previous = _read_keychain_secret_strict(name)
        if previous == value:
            return
        try:
            keyring.set_password(KEYCHAIN_SERVICE, name, value)
        except Exception as exc:
            if _keychain_error_status(exc) != -25244:
                raise
            _update_keychain_secret_in_place(name, value)
        if _read_keychain_secret_strict(name) != value:
            raise RuntimeError("钥匙串写入后校验不一致")
    except Exception as exc:
...
def get_keychain_secret(name):
    name = validate_env_name(name)
    try:
        return _read_keychain_secret_strict(name)
    except Exception:
        return None
...
def delete_keychain_secret(name):
    name = validate_env_name(name)
    try:
        if _read_keychain_secret_strict(name) is None:
            return
        keyring.delete_password(KEYCHAIN_SERVICE, name)
    except keyring.errors.PasswordDeleteError as exc:
        if _keychain_error_status(exc) == -25300:
            return
        raise RuntimeError("删除 macOS 钥匙串密钥失败：%s" % exc)


def restore_keychain_secret(name, value):
    """仅在当前值确实改变时回滚，避免旧 ACL 上重复写入再次失败。"""
    name = validate_env_name(name)
    current = get_keychain_secret(name)
...
def _codex_app_executable(bundle):
    """从 Codex.app 的 Info.plist 解析真正的主程序，避免 LaunchServices 丢失环境和参数。"""
    plist_path = os.path.join(bundle, "Contents", "Info.plist")
    try:
        with open(plist_path, "rb") as handle:
            info = plistlib.load(handle)
        executable = str(info.get("CFBundleExecutable") or "Codex").strip()
...
def _codex_login_launch_args():
    """降低 Electron 在 Intel/旧 macOS 图形路径上的白屏概率。"""
    return [
        "--disable-gpu",
...
def _terminate_residual_codex_pids(old_pids):
    """清理本次启动产生但未稳定起来的 Codex 进程。"""
    residual = codex_gui_pids() - set(old_pids or ())
    for pid in residual:
        try:
            os.kill(pid, 15)
        except (ProcessLookupError, PermissionError):
...
def _activate_codex_frontmost():
    """将 Codex 窗口置前；不依赖启动命令是否创建了新进程。"""
    try:
        subprocess.run(
...
def launch_official_login_app(old_pids=None):
    """直接启动官方 App 主程序，显式传递干净环境并绕过已知 GPU 白屏路径。"""
    old_pids = set(old_pids or ())
    bundles = [path for path in _codex_app_bundles() if os.path.isdir(path)]
    bundle = bundles[0] if bundles else ""
    executable = _codex_app_executable(bundle) if bundle else ""
    if not executable:
        raise RuntimeError("未找到 Codex.app 主程序，请将官方 Codex.app 放入 /Applications")
    env = official_login_environment()
    result = subprocess.Popen(
        [executable] + _codex_login_launch_args(),
    ...
    if not fresh:
        _terminate_residual_codex_pids(old_pids)
        raise RuntimeError("Codex 官方登录窗口未能稳定启动，残留新进程已清理（启动 PID：%s）" % result.pid)
    _activate_codex_frontmost()
    return [
        "已直接启动官方 Codex.app 并禁用异常 GPU 合成路径，新登录窗口已稳定运行（PID：%s）"
...
def launch_fresh_codex_app(cli, cwd, env, old_pids=None):
    """为第三方认证直接启动官方 App 主程序，避免 codex app/LaunchServices 复用白屏实例。"""
    del cli, cwd
    old_pids = set(old_pids or ())
    app_bundles = [path for path in _codex_app_bundles() if os.path.isdir(path)]
    bundle = app_bundles[0] if app_bundles else ""
    executable = _codex_app_executable(bundle) if bundle else ""
    if not executable:
        raise RuntimeError("未找到 Codex.app 主程序，请将官方 Codex.app 放入 /Applications")
    ...
    if not fresh:
        _terminate_residual_codex_pids(old_pids)
        raise RuntimeError("Codex 新 GUI 未能稳定启动，残留新进程已清理（启动 PID：%s）" % process.pid)
    _activate_codex_frontmost()
    return [
        "已直接启动官方 Codex.app 并禁用异常 GPU 合成路径（PID：%s），认证状态已重新加载"
        % ", ".join(map(str, sorted(fresh)))
    ]
...
def main():
    if "--self-test-package" in sys.argv:
        ...
        if APP_VERSION != "1.24-macOS":
            raise RuntimeError("应用版本不是 V1.24 macOS")
        if sys.platform == "darwin":
            ...
            if not callable(_update_keychain_secret_in_place):
                raise RuntimeError("macOS 钥匙串原地更新能力不可用")
